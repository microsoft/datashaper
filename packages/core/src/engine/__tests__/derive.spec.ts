...
describe('test for derive verb', () => {
	...
	test('derive test with MathOperator Substract and string - number', () => {
		...
		return derive(step, store).then(result => {
			...
			// no new rows
			expect(result.numRows()).toBe(5)
			expect(result.get('newColumn', 0)).toBe(NaN)
			expect(result.get('newColumn', 1)).toBe(NaN)
			expect(result.get('newColumn', 2)).toBe(NaN)
			expect(result.get('newColumn', 3)).toBe(NaN)
			expect(result.get('newColumn', 4)).toBe(NaN)
		})
	})
