...
def _destructureSingleValue(row, object: object, isArray: bool, prefix: str, keys: List[str] = []):
    index = 0
...
def destructure(input: VerbInput, column: str, keys: List[str] = [], prefix: str = "array_", preserveSource: bool = False):
    ...
    for index in output.index:
        if output[column][index] != None:
            ...
            output.loc[index] = _destructureSingleValue(output.loc[index], output[column][index], type(output) is list, prefix, keys)
