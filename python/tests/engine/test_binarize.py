...
def test_binarize_numeric_gte():
    ...
    assert len(result) == 5
    assert result.loc[0, "newColumn"] == 0
    assert result.loc[1, "newColumn"] == 0
    assert result.loc[2, "newColumn"] == 0
    assert result.loc[3, "newColumn"] == 1
    assert result.loc[4, "newColumn"] == 1
...
def test_binarize_numeric_gt():
    ...
    assert result.loc[1, "newColumn"] == 0
    assert result.loc[2, "newColumn"] == 0
    assert result.loc[3, "newColumn"] == 0
    assert result.loc[4, "newColumn"] == 1
...
def test_binarize_numeric_lt():
    ...
    assert len(result.columns) == 4
    assert len(result) == 5
    assert result.loc[0, "newColumn"] == 1
    assert result.loc[1, "newColumn"] == 1
    assert result.loc[2, "newColumn"] == 1
    assert result.loc[3, "newColumn"] == 0
    assert result.loc[4, "newColumn"] == 0
...
def test_binarize_string_endswith():
    ...
    assert result.loc[3, "newColumn"] == 1
    assert result.loc[4, "newColumn"] == 0
    assert result.loc[5, "newColumn"] == 0
...
def test_binarize_string_empty():
    ...
    assert result.loc[3, "newColumn"] == 0
    assert result.loc[4, "newColumn"] == 0
    assert result.loc[5, "newColumn"] == 1
