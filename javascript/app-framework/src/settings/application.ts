 ...
 */
import { atom, useRecoilValue, useRecoilState, useSetRecoilState, type SetterOrUpdater } from 'recoil'
import { useCallback } from 'react'

const LOCAL_STORAGE_KEY = 'app-framework-application-settings'
const DEFAULT = {}

const storageEffect = (key: string) => ({ setSelf, onSet}: { setSelf: any, onSet: any }) => {
 ...
 */
export function useSetDefaultApplicationSettings(): (settings: unknown) => void {
	const setter = useSetRecoilState(state)
	return useCallback((defaults: unknown) => {
		setter((prev: unknown) => {
