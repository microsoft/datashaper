...
import {
	createDataBundleSchemaObject,
	LATEST_DATABUNDLE_SCHEMA,
...
export class DataBundle
	extends Resource
	implements SchemaResource<DataBundleSchema>
{
	public readonly $schema = LATEST_DATABUNDLE_SCHEMA
	public readonly profile = 'databundle'
	public readonly defaultName = 'databundle.json'
	...
	public override toSchema(): DataBundleSchema {
		return createDataBundleSchemaObject({
			...super.toSchema(),
		})
	}

	public override loadSchema(
		schema: Maybe<DataBundleSchema>,
		quiet?: boolean,
	): void {
	...
	public connect(dp: DataPackage): void {
		const rebindInputs = () => {
			...
			tableNames.forEach(name =>
				this._inputs.set(
					name,
					(dp.getResource(name) as DataBundle)?.output$ ??
